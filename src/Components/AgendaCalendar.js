            ...
            <View style={{ flexDirection: 'row' }}>
                <View style={{ flex: 3 }}>
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            navigation.navigate('Save Event Main', { screen: 'Save Event', params: { eventId: item.id } });
                        }} //일정 수정
                    >
                        <Text>{item.name}</Text>
                        <Text>{moment(item.during.start).format('MM/DD  HH:mm')} ~</Text>
                        <Text>{moment(item.during.end).format('MM/DD  HH:mm')}</Text>
                    </TouchableOpacity>
                </View>
                <View style={{ flex: 1 }}>
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            remove(item.id, item.during.start);
                        }}>
    ...
    return (
        <View style={{ height: '100%' }}>
            ...
            <Fab
                ...
                onPress={() => {
                    navigation.navigate('Save Event Main');
                }}>
